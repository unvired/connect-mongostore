...
function getStore(Store) {
  ...
  MongoStore.prototype.getCollection = function (cb) {
    ...
    if (!self.db.serverConfig.isConnected()) return self.getDatabase(cb)
    ...
    self.db.collection(self.collectionName, function (err, collection) {
      ...
      self.collection.createIndex({'expires': 1}, {'expireAfterSeconds': 0}, function (err, result) {
...
function setupDb(opts) {
  ...
  if (Array.isArray(opts.db.servers)) {
    ...
    return new mongo.Db(opts.db.name, new mongo.ReplSet(serverArray), opts.db.replicaSetOptions || {'w': 1})
  }
